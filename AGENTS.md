...
Sources/
  BrainDumpKit/           ← library: models, persistence, services, AppState, views
    Models/               ← @Model classes (Day, TaskItem, ScheduleEntry) + TodoError + drag payload; calendar value types (CalendarEvent, CalendarFeed — Codable structs, NOT SwiftData)
    Persistence/          ← PersistenceController (corruption-recovering container; versioned schema BrainDumpSchemaV2, lightweight auto-migration — no formal plan) + CalendarCache (calendar-cache.json)
    Services/             ← business logic: Day / Task / Schedule / Backlog / Backup; Calendar (CalendarService, CalendarFeedStore, ICalParser, ICalDate, ICalFeedFetcher, RecurrenceRule, RecurrenceExpander); Notification planners (NotificationCoordinator, ScheduleReminderPlanner, BacklogDigestPlanner)
    Notifications/        ← UserNotifying protocol + SystemUserNotifying (UNUserNotificationCenter) test seam
    App/                  ← AppState (@Observable; selected date, sidebar destination, day-bounds + notification prefs) + AppUpdateModel (Sparkle-free update surface; app target wires real Sparkle, Kit never links it)
    Views/                ← AppShell (sidebar + canvas), DayView + sections (Top3/BrainDump/Schedule), Tasks/Backlog screens, TimeBlockSheet, TaskDetailSheet, SettingsSheet, CalendarSettingsView, UpdatesSettingsView, MonthCalendarView, calendar event views (CalendarEventBlockView, AllDayEventBar), TimeRangePicker, tag/chip/row helpers
    Support/              ← Date+StartOfDay, TimeFormat, ScheduleDefaults, ReminderOffset, Theme, Fonts, WindowSizing, WiseSayings
    Resources/Fonts/      ← bundled variable fonts (Hanken Grotesk, Source Serif 4), registered by Fonts.swift
Tests/
  BrainDumpTests/         ← Swift Testing target (XCTest is NOT used); VisualSnapshotTests renders PNGs
    TestSupport/          ← InMemoryStore + FileBackedStore (on-disk) + TestDate + SpyUserNotifying helpers
```
...
- **App-level settings** live on `AppState`: `dayStartHour`/`dayEndHour` (default 5/22, persisted to `UserDefaults`, validated to span ≥ 4 hours) and `isSidebarVisible` (in-memory only). Backlog-digest prefs (`backlogDigestEnabled`/`backlogDigestThresholdDays`/`backlogDigestHour`/`backlogDigestMinute`) are persisted to `UserDefaults` here too. Views read these directly; there's no separate settings object.
...
**Never crashes on launch.** `PersistenceController.makeContainer()` opens the versioned store (`BrainDumpSchemaV2`, built with `Schema(versionedSchema:)` and **no formal migration plan** — SwiftData's lightweight auto-migration covers purely additive/optional changes); on failure it moves the unreadable store and its `-wal`/`-shm` sidecars aside (`BrainDump.store.corrupt-<unixstamp>`, **preserved, not deleted**) and retries fresh; last resort is an in-memory container. The outcome is a `StoreRecovery` value surfaced once as an alert in `AppShell`. Store path: `~/Library/Application Support/<dir>/BrainDump.store`, where `<dir>` is `PersistenceController.appDirectoryName` — `BrainDump` for Release and `BrainDump-debug` for Debug builds, so a dev build never clobbers the installed app's real data (the calendar cache shares the same directory). Purely additive, optional schema changes need only a bumped `BrainDumpSchemaV2` (or a new `VersionedSchema`) — no `MigrationStage`. A hand-written migration plan previously crashed launch on upgrade and was removed (`972bc1d`); reintroduce one only if a non-additive change genuinely requires it.
...
**Calendar (read-only iCal subscriptions).** Each `CalendarFeed` (name + URL + colorIndex + isEnabled, stored via `CalendarFeedStore`) is fetched by `URLSessionICalFeedFetcher` (`webcal://` → `https://`), parsed by `ICalParser` (line unfolding, `RRULE`/`EXDATE`), and expanded within the visible window by `RecurrenceExpander` — including Google-style orphan `RECURRENCE-ID` overrides that have no master event. `CalendarService` merges overlapping busy ranges and caches results to `calendar-cache.json` in the app-support dir (shared with the store; see `CalendarCache`). Calendar events are **display-only overlays** on the schedule grid — never `TaskItem`s or `ScheduleEntry`s.

**Local notifications.** `NotificationCoordinator` sits behind the `UserNotifying` protocol (real `SystemUserNotifying` wraps `UNUserNotificationCenter`; tests inject `SpyUserNotifying`) and syncs two kinds: per-entry **schedule reminders** (`ScheduleReminderPlanner` + `ReminderOffset`, persisted on `ScheduleEntry.reminderOffsetMinutes`; skips past or completed entries) and a **backlog-age digest** (`BacklogDigestPlanner`, fires daily at `backlogDigestHour`/`Minute` when backlog items exceed `backlogDigestThresholdDays`, gated by `backlogDigestEnabled`). Planners are pure functions of an injected `now`, so they test without the system notification center.
